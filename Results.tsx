...
export default function Results() {
  ...
  // --- دالة الطباعة (المحدثة لتكرار المعلومات في كل صفحة) ---
  const printResults = () => {
    ...
    // --- هذا "قالب" معلومات المريض الذي سيتكرر في كل صفحة ---
    ...
    // الفحوصات الطويلة (مثل اليورين) تأخذ صفحة كاملة
    longTests.forEach(test => pages.push([test]));

    // الفحوصات القصيرة: تقسم كل 13 فحص في صفحة
        ...
        <head>
          ...
          <style>
            * { margin: 0; padding: 0; box-sizing: border-box; }
            ...
            /* كلاس الصفحة: يضمن فاصل الصفحات وتوزيع المحتوى */
            .page-container { 
            ...
            /* تنسيق الأقسام المخصصة (فوق وتحت) */
        ...
        <body>
          ${pages.map((pageTests, index) => `
            <div class="page-container">
               ...
               <!-- 1. الكتابة العلوية (من الضبط) - تكرر في كل صفحة -->
               ${topSections.map(s => `
                 <div class="custom-section top" style="text-align:${s.alignment};color:${s.textColor};background:${s.backgroundColor};font-size:${s.fontSize}px">
                   ${escapeHtml(s.text)}
                 </div>
               `).join('')}
               
               <!-- 2. معلومات المريض - تكرر في كل صفحة -->
               ${patientHeaderHTML}
               ...
               <!-- 3. الكتابة السفلية (من الضبط) - تكرر في كل صفحة -->
               ${bottomSections.map(s => `
                 <div class="custom-section bottom" style="text-align:${s.alignment};color:${s.textColor};background:${s.backgroundColor};font-size:${s.fontSize}px">
                   ${escapeHtml(s.text)}
                 </div>
               `).join('')}
            </div>
