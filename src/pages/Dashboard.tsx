...
export default function Dashboard() {
  ...
  const initLayoutsMutation = useMutation({
    ...
    onSuccess: () => {
      ...
      toast({
        title: "Initialized",
        description: "Dashboard layouts initialized successfully",
      });
    },
  });
  ...
  const handleDragStop = (newLayout: any[]) => {
    ...
    newLayout.forEach((item) => {
      ...
      if (layout && (layout.positionX !== item.x || layout.positionY !== item.y)) {
        commitLayoutChange(layout.sectionName, {
          positionX: item.x,
          positionY: item.y,
          width: item.w,
          height: item.h,
  ...
  const gridLayout = localLayouts.map(layout => ({
        ...
        <header className="border-b border-border bg-card/50 backdrop-blur-sm sticky top-0 z-10">
          <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
            <div className="flex items-center gap-3">
              ...
              <div>
                <h1 className="text-xl font-semibold text-foreground">KAROZH</h1>
    ...
    <div className="min-h-screen bg-background">
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-4 flex items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2">
            <Button
              ...
              title={isLocked ? "Unlock to customize" : "Lock customization"}
            >
              {isLocked ? (
          ...
          {localLayouts.map((layout) => {
            ...
            return (
              <div key={layout.sectionName}>
                <Card
                  className={`${isLocked ? "cursor-pointer hover-elevate active-elevate-2" : "cursor-move"} transition-all overflow-hidden relative h-full flex flex-col`}
                  onClick={isLocked ? () => setLocation(layout.route) : undefined}
                  data-testid={`card-${layout.sectionName}`}
                >
                  ...
                  {!isLocked && (
                    <Button
                      variant="ghost"
                      size="icon"
                      className="edit-button absolute top-2 right-2 z-10"
                      style={{ width: `${editButtonSize}px`, height: `${editButtonSize}px`, minHeight: `${editButtonSize}px` }}
                      onClick={(e) => handleEditClick(layout, e)}
                      data-testid={`button-edit-${layout.sectionName}`}
                    >
                  ...
                  <div className="flex flex-col items-center justify-center gap-3 w-full flex-1 p-4">
                    ...
                    <div className="text-center w-full px-2">
                      <h3 className="font-semibold leading-tight" style={{ fontSize: `${fontSize}px` }}>{layout.displayName}</h3>
                    </div>
                  </div>
                </Card>
              </div>
      ...
      <Dialog open={!!editingSection} onOpenChange={() => setEditingSection(null)}>
        <DialogContent data-testid="dialog-edit-widget">
            ...
            <div className="space-y-2">
              ...
              <div className="grid grid-cols-2 gap-2">
                <Button
                  ...
                  onClick={() => setEditForm({ ...editForm, color: "from-blue-500/10 to-blue-500/5" })}
                  data-testid="button-color-blue"
                >
                  <div className="w-4 h-4 rounded bg-gradient-to-br from-blue-500/10 to-blue-500/5 mr-2" />
                  Blue
                </Button>
                <Button
                  variant="outline"
                  className="justify-start"
                  onClick={() => setEditForm({ ...editForm, color: "from-green-500/10 to-green-500/5" })}
                  data-testid="button-color-green"
                >
                  <div className="w-4 h-4 rounded bg-gradient-to-br from-green-500/10 to-green-500/5 mr-2" />
                  Green
                </Button>
                <Button
                  variant="outline"
                  className="justify-start"
                  onClick={() => setEditForm({ ...editForm, color: "from-purple-500/10 to-purple-500/5" })}
                  data-testid="button-color-purple"
                >
                  <div className="w-4 h-4 rounded bg-gradient-to-br from-purple-500/10 to-purple-500/5 mr-2" />
                  Purple
                </Button>
                <Button
                  variant="outline"
                  className="justify-start"
                  onClick={() => setEditForm({ ...editForm, color: "from-amber-500/10 to-amber-500/5" })}
                  data-testid="button-color-amber"
                >
                  <div className="w-4 h-4 rounded bg-gradient-to-br from-amber-500/10 to-amber-500/5 mr-2" />
                  Amber
                </Button>
                <Button
                  variant="outline"
                  className="justify-start"
                  onClick={() => setEditForm({ ...editForm, color: "from-red-500/10 to-red-500/5" })}
                  data-testid="button-color-red"
                >
                  <div className="w-4 h-4 rounded bg-gradient-to-br from-red-500/10 to-red-500/5 mr-2" />
                  Red
                </Button>
                <Button
                  variant="outline"
                  className="justify-start"
                  onClick={() => setEditForm({ ...editForm, color: "from-gray-500/10 to-gray-500/5" })}
                  data-testid="button-color-gray"
                >
                  <div className="w-4 h-4 rounded bg-gradient-to-br from-gray-500/10 to-gray-500/5 mr-2" />
                  Gray
                </Button>
              </div>
            </div>
          ...
          <DialogFooter>
            <Button variant="outline" onClick={() => setEditingSection(null)} data-testid="button-cancel">
              Cancel
            </Button>
            <Button onClick={handleSaveEdit} className="gap-2" data-testid="button-save-widget">
